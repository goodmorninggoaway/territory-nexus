...
import AlbaToTerritoryHelperSyncContainer from './sync/AlbaToTerritoryHelperSyncContainer';
import AlbaToTerritoryHelperApiSyncContainer from './sync/AlbaToTerritoryHelperApiSyncContainer';
import ManualEntryToTerritoryHelperSyncContainer from './sync/ManualEntryToTerritoryHelperSyncContainer';
    ...
    getNav() {
        return [
            { name: 'My Congregation', icon: <Group />, url: '/my-congregation', component: CongregationContainer },
            {
                ...
                url: '/sync',
                exact: true,
                component: () => <Redirect to="/sync/alba-address-to-th-location" />,
                children: [
                    { name: 'Alba -> Territory Helper', url: '/sync/alba-address-to-th-location', component: AlbaToTerritoryHelperSyncContainer },
                    {
                        name: 'Alba -> Territory Helper (Automatic)',
                        url: '/sync/alba-address-to-th-api-location',
                        component: AlbaToTerritoryHelperApiSyncContainer,
                    },
                    {
                        name: 'Manual Entry -> Territory Helper',
                        url: '/sync/manual-address-to-th-location',
                        component: ManualEntryToTerritoryHelperSyncContainer,
                    },
                ],
            },
                ...
                >
                    {this.getNav().map(({ name, url, children }) => (
                        <>
                            ...
                            {children && children.map(({ name, url }) => (
                                <RoutedButton key={name} path={url} hoverIndicator>
                                    <Box pad={{ horizontal: 'medium', vertical: 'small' }} direction="row">
